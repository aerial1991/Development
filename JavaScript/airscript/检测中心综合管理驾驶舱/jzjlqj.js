...
let range = Application.Sheets("计量器具检校动态").UsedRange
...
const result2 = connection.queryAll(
  'TRUNCATE TABLE calibration_process'
)
// 打印执行结果
console.log(result2)
...
const result3 = connection.queryAll(
  'INSERT INTO calibration_process (ID, Instrument_Name, Model, Instrument_Number, Commission_Date, Phase,Report_Date,Conclusion) VALUES (?,?,?,?,?,?,?,?)',
  [res[i][0],res[i][2],res[i][3],res[i][4],res[i][5],res[i][6],res[i][7],res[i][8]]
)
console.log(result3)
}
// const result4 = connection.queryAll(
//   'UPDATE calibration_process SET Report_Date = NULL WHERE Report_Date = "1899-12-30"'
// )
